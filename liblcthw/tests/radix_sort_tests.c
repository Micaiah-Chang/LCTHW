...
static int check_order(RadixMap *map)
{
	 ...
	 for(i = 0; map->end > 0 && i < map->end-1; i++) {
		  ...
		  if(d1.data.key > d2.data.key) {
			   debug("FAIL:i=%u, key: %u, value: %u, equals max? %d", i, d1.data.key, d1.data.value,
...
static char *test_operations()
{
	 ...
	 mu_assert(check_order(map), "RadixMap didn't stay sorted after serach");
	 ...
	 while(map->end > 0) {
		  ...
		  mu_assert(check_order(map), "RadixMap didn't stay sotred after delete.");
	 }
